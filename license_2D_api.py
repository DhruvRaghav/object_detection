...
import blur
from license_2D_detect import run
...
@app.route('/lp_box', methods=['POST', 'GET'])  # Single Api
def box():
...
@app.route('/lp_blur', methods=['POST', 'GET'])  # Single Api
def label():
    resp = Response(status=200, content_type='application/json')
...
@app.route('/lp_draw', methods=['POST', 'GET'])  # Single Api
def box_img():
    resp = Response(status=200, content_type='application/json')
