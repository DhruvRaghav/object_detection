...
@app.route('/face_box', methods=['POST', 'GET'])  # Single Api
def box():
...
@app.route('/face_blur', methods=['POST', 'GET'])  # Single Api
def label():
    resp = Response(status=200, content_type='application/json')
            ...
            if request.content_type.startswith('multipart/form-data'):
                if 'file' in request.files.keys():
                    ...
                    if (request.files['file'].filename.endswith('.jpg')) or (
                        ...
                        blur.rectblur_NEW('API_DATA/box_label/test.png', result)
...
@app.route('/face_draw', methods=['POST', 'GET'])  # Single Api
def box_img():
    resp = Response(status=200, content_type='application/json')
