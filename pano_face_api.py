...
@app.route('/face_pano_box', methods=['POST', 'GET'])  # Single Api
def box():
        ...
        if request.content_type is not None:
            if request.content_type.startswith('multipart/form-data'):
                if 'file' in request.files.keys():
                    if (request.files['file'].filename.lower().endswith('.jpg')) or (
                        ...
                        result = run(source='API_DATA/box', nosave=True, box=True, device=0)
...
@app.route('/face_pano_blur', methods=['POST', 'GET'])  # Single Api
def label():
    resp = Response(status=200, content_type='application/json')
        ...
        if request.content_type is not None:
            if request.content_type.startswith('multipart/form-data'):
                if 'file' in request.files.keys():
                    ...
                    if (request.files['file'].filename.lower().endswith('.jpg')) or (
                            request.files['file'].filename.lower().endswith('.png')) or (
                            request.files['file'].filename.lower().endswith('.jpeg')):
                        ...
                        with open("bluroutput/image.png", "rb") as image2string:
...
@app.route('/face_pano_draw', methods=['POST', 'GET'])  # Single Api
def box_img():
    resp = Response(status=200, content_type='application/json')
